  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (fontFamily === 'inherit') {
        const computed = window.getComputedStyle(canvas).fontFamily || 'sans-serif';
        // Remove CSS variables which canvas context font string does not support (e.g. var(--font-inter))
        computedFontFamily = computed.replace(/var\(--[^)]+\),?\s*/g, '').trim() || 'sans-serif';
        computedFontFamily = computedFontFamily.replace(/^,\s*|,\s*$/g, '').trim() || 'sans-serif';
      }
      ...
      const run = (timestamp: number) => {
        ...
        if (direction === 'horizontal') {
          ...
          // Both: shift each row horizontally, and column vertically using drawImage (high-performance GPU path)
          const sliceWidth = 2;
          for (let i = 0; i < offscreenWidth; i += sliceWidth) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange * 0.4);
            ctx.drawImage(offscreen, i, 0, sliceWidth, tightHeight, i + dx, dy, sliceWidth, tightHeight);
          }
        }
        animationFrameId = window.requestAnimationFrame(run);
      };
